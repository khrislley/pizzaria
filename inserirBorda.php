<?php
include_once("db.php");
?>

<html>

<head>
    <title>PIZZARIA</title>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="./style.css">
</head>

<body>

    <?php
    session_start();
    if(!isset($_SESSION['login'])){
        echo "ERRO! O usuário não está logado!";
        exit();
    }
    ?>

    <header>
        <h1>CADASTRO DE BORDAS</h1>
    </header>

    <div class="conteudo">
        <div class="cadastrar"><br><br>
            <form action="inserirBorda.php">
                <label for="nomeBorda">Borda: </label><input type="text" name="nomeBorda" id="nomeBorda"> <br><br>
                <input type="submit" value="Cadastrar"> <br><br>
            </form>


            <?php
            if (isset($_REQUEST["nomeBorda"])) {
                $nomeBorda = $_REQUEST["nomeBorda"];

                $queryBorda = $conexao->query("INSERT INTO BORDA(NOME_BORDA) VALUES ('$nomeBorda')");

                echo "<br> Borda cadastrada com sucesso! <br>";
            }
            ?>

        </div><br>
        <a href="menu.php">Página inicial</a>
    </div>
    <footer id="footer" style="font-size: 16px">
        Copyright ©2020 Khrislley Oliveira - Todos os direitos reservados.
    </footer>


</body>

</html>